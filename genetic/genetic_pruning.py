...
class GeneticPruning:
        ...
        with torch.no_grad():
            for i in range(nsamples):
                try:
                    ...
                    # Only print errors during actual PPL evaluation, not during model building
                    print(f"    Error in evaluation sample {i} on {device}: {e}")
                    raise
    ...
    def evolve(self, resume_from: str = None) -> Individual:
        ...
        if resume_from:
            ...
            # Use tqdm for progress bar during initial evaluation
            population = self.evaluate_fitness_batch_parallel(
                tqdm(population, desc="Initial Eval", leave=False)
            )
        ...
        # Evolution Loop
        for generation in tqdm(range(start_generation, self.max_generations), desc="Evolution"):
            new_population = []
            ...
            if current_best:
                ...
                tqdm.write(f"  👑 Elitism: Kept best (Fitness: {current_best.fitness:.4f})")
            else:
                ...
                tqdm.write(f"  ⚠️ Elitism: No valid individuals. Kept best invalid (Fitness: {fallback_best.fitness:.4f})")

            # 2. Generate Offspring
            # We need to fill population_size - len(new_population) spots
            offspring_to_evaluate = []

            for _ in tqdm(range((self.population_size - len(new_population)) // 2), desc=f"Gen {generation+1} Offspring", leave=False):
                # Selection
            ...
            # Ensure correct population size in case of odd remaining slots
            remaining_slots = self.population_size - len(new_population)
            offspring_to_evaluate = offspring_to_evaluate[:remaining_slots]

            # 3. Evaluate Offspring
            tqdm.write(f"  🧬 Evaluating {len(offspring_to_evaluate)} offspring...")
            offspring_to_evaluate = self.evaluate_fitness_batch_parallel(
                tqdm(offspring_to_evaluate, desc=f"Gen {generation+1} Eval", leave=False)
            )
            ...
            if gen_best.is_valid and gen_best.fitness < best_ever.fitness:
                tqdm.write(f"  🎉 New Global Best Found! Fitness: {gen_best.fitness:.4f} (was {best_ever.fitness:.4f})")
                best_ever = copy.deepcopy(gen_best)
            else:
                tqdm.write(f"  Generation Best (Valid): {gen_best.fitness:.4f} (Global Best: {best_ever.fitness:.4f})")
            ...
            tqdm.write(f"  Stats: Valid={valid_count}/{self.population_size}, Avg Valid Fitness={avg_fitness:.4f}")
            tqdm.write(f"  Best Individual for this gen: {gen_best}")
        ...
        tqdm.write("="*80)
        tqdm.write(f"Best Individual Found (Final): {best_ever}")
